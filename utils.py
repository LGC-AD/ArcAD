...
def min_max_norm_tensor(x):
    """
    对 Tensor 进行 Batch 内的 Min-Max 归一化
    x: [B, 1, H, W]
    """
    B, C, H, W = x.shape
    x_flat = x.view(B, -1)
...
def evaluation_fusion(model, discriminator, dataloader, device, w_recon=0.7, w_disc=0.3, num_special_tokens=5):
    """
    融合评估函数 (Plan B)
    """
    model.eval()
    ...
    # 使用 utils 中已有的 get_gaussian_kernel 生成平滑卷积层
    # sigma=4 是 SimpleNet/EfficientAD 等论文推荐的值
    gaussian_filter = get_gaussian_kernel(kernel_size=33, sigma=4, channels=1).to(device)
    ...
    # 专门保留原始形状的 map 和 mask 用于计算 AUPRO (比较耗内存，但必要)
    all_maps = []
    all_masks = []
    ...
    with torch.no_grad():
        for img, gt, label, _ in dataloader:
            ...
            # --- 1. Forward Pass ---
            # 获取 Encoder 和 Decoder 的特征
            output = model(img)
            en, de, blk = output[0], output[1], output[2]

            # --- 2. Reconstruction Map Calculation ---
            # 使用现有的 cal_anomaly_maps 计算重构图
            # recon_map: [B, 1, H, W]
            recon_map, _ = cal_anomaly_maps(en, de, img.shape[-1])
            
            # --- 3. Discriminator Map Calculation ---
            # 提取 Patch 特征 (去掉 CLS 等特殊 Token)
            tokens = blk[:, num_special_tokens:, :] # [B, N_patches, C]
            B, N, C = tokens.shape
            H_feat = int(math.sqrt(N)) # e.g. 14

            # 归一化特征并输入判别器
            tokens_norm = F.normalize(tokens, dim=-1).reshape(-1, C)
            logits = discriminator(tokens_norm)
            probs = torch.sigmoid(logits) # 属于"正常"的概率
            disc_score = 1 - probs        # 属于"异常"的概率
            
            # Reshape 回空间维度 [B, 1, H_feat, W_feat]
            disc_map = disc_score.view(B, 1, H_feat, H_feat)
            
            # 双线性插值放大到原图尺寸
            disc_map = F.interpolate(disc_map, size=img.shape[2:], mode='bilinear', align_corners=False)
            
            # [关键] 高斯平滑消除棋盘格效应
            disc_map = gaussian_filter(disc_map)

            # --- 4. Normalization & Fusion ---
            # 对两者进行 Min-Max 归一化，确保权重 0.7/0.3 有效
            recon_map_norm = min_max_norm_tensor(recon_map)
            disc_map_norm = min_max_norm_tensor(disc_map)
            
            # 加权融合
            final_map = w_recon * recon_map_norm + w_disc * disc_map_norm

            # --- 5. Data Collection ---
            # 处理 GT Mask (Binarize)
            if gt.shape[1] > 1:
                gt = torch.max(gt, dim=1, keepdim=True)[0]
            gt = gt.bool().float() # 确保是 0/1 float

            # 图片级分数: 取该图 Map 的最大值
            sp_score = torch.max(final_map.flatten(1), dim=1)[0]
            
            # 收集用于计算指标的数据
            gt_list_sp.append(label.cpu().numpy())
            pr_list_sp.append(sp_score.cpu().numpy())
            
            # 像素级 (Flatten)
            gt_list_px.append(gt.cpu().numpy().ravel())
            pr_list_px.append(final_map.cpu().numpy().ravel())
            
            # 为 AUPRO 保留完整形状 (Squeeze channel dim)
            all_maps.append(final_map.squeeze(1).cpu().numpy())
            all_masks.append(gt.squeeze(1).cpu().numpy())

    # --- 6. Metrics Calculation ---
    gt_list_sp = np.concatenate(gt_list_sp)
    pr_list_sp = np.concatenate(pr_list_sp)
    gt_list_px = np.concatenate(gt_list_px)
    pr_list_px = np.concatenate(pr_list_px)
    
    # Image Metrics
    auroc_sp = roc_auc_score(gt_list_sp, pr_list_sp)
    ap_sp = average_precision_score(gt_list_sp, pr_list_sp)
    f1_sp = f1_score_max(gt_list_sp, pr_list_sp) # 使用 utils 中已有的函数
    
    # Pixel Metrics (AUROC, AP, F1)
    ...
    # Pixel Metrics (AUPRO)
    all_maps_np = np.concatenate(all_maps, axis=0)
    all_masks_np = np.concatenate(all_masks, axis=0)
    
    # 只有当存在正样本(异常)时才计算 AUPRO
    if all_masks_np.sum() > 0:
        aupro_px = compute_pro(all_masks_np, all_maps_np)
    else:
        aupro_px = 0.0
...
# def global_cosine_hm_percent(a, b, p=0.9, factor=0.):
#     """
#     a: list of tensors (Encoder outputs)
#     b: list of tensors (Decoder/Bottleneck outputs)
#     p: 保留梯度的比例 (Hard Mining)
#     factor: 被过滤掉的样本的梯度权重 (通常为 0)
#     """
#     loss = 0
#     count = 0
    
#     for item in range(len(a)):
#         # 1. 获取输入并强制转为 Float32 (解决 NaN 的核心)
#         # a[item] 是 target (通常 detach)
#         a_src = a[item]
#         b_src = b[item]

#         # 转换为 float32 计算，防止 FP16 溢出
#         a_f = a_src.float().detach()
#         b_f = b_src.float()

#         with torch.no_grad():
#             # 2. 计算逐像素/特征点的 Cosine Distance
#             # 使用 eps=1e-5 防止除以 0
#             # dim=1 通常是 Channel 维度
#             sim_map = F.cosine_similarity(a_f, b_f, dim=1, eps=1e-5)
            
#             # point_dist shape: [B, 1, H, W]
#             point_dist = 1 - sim_map.unsqueeze(1)

#             # [安全措施] 清理计算过程中可能产生的 NaN
#             if torch.isnan(point_dist).any():
#                 point_dist = torch.nan_to_num(point_dist, nan=0.0, posinf=1.0, neginf=0.0)

#             # 3. 计算 Threshold (Hard Mining)
#             # 展平后找 TopK
#             dist_flat = point_dist.reshape(-1)
#             # 计算要保留的元素数量
#             k_val = int(dist_flat.numel() * (1 - p))
            
#             if k_val > 0:
#                 # 找到第 k 大的值作为阈值
#                 topk_val, _ = torch.topk(dist_flat, k=k_val)
#                 thresh = topk_val[-1]
#             else:
#                 thresh = dist_flat.max()

#         # 4. 计算实际用于反向传播的 Loss
#         # 原逻辑是将 [B, C, H, W] 展平为 [B, C*H*W] 计算全局相似度
#         a_flat = a_f.flatten(1)
#         b_flat = b_f.flatten(1)

#         # 计算 Global Cosine Loss
#         sim_global = F.cosine_similarity(a_flat, b_flat, dim=1, eps=1e-5)
#         loss_step = 1 - sim_global
#         loss += loss_step.mean()

#         # 5. 注册 Hook (Gradient Masking)
#         # 只有距离大于阈值 (难样本) 的地方保留梯度，距离小 (Easy) 的地方梯度 * factor
#         # mask = True 的地方是 Easy Samples (dist < thresh)
#         mask = point_dist < thresh

#         if factor < 1.0 and b_f.requires_grad:
#             # 注意：这里 hook 注册在 b_f 上，因为 loss 是由 b_f 算出来的
#             # 梯度会从 b_f 传回 b_src
#             partial_func = partial(modify_grad, inds=mask, factor=factor)
#             b_f.register_hook(partial_func)
            
#         count += 1

#     if count == 0:
#         return torch.tensor(0.0, device=a[0].device, requires_grad=True)
...
def compute_pro(masks: ndarray, amaps: ndarray, num_th: int = 200) -> float:
    """
    计算 AUPRO (Area Under Per-Region Overlap)
    masks: (N, H, W), 0/1 binary
    amaps: (N, H, W), float anomaly scores
    """
    # 确保输入类型
    ...
    if max_val - min_val > 1e-8:
        amaps = (amaps - min_val) / (max_val - min_val)
    else:
        amaps = np.zeros_like(amaps)

    # 2. 生成阈值
    thresholds = np.linspace(0, 1, num_th)
    
    # 3. 预先计算总的正常像素数 (用于计算 FPR)
    # inverse_masks = 1 - masks
    # total_neg_pixels = inverse_masks.sum()
    ...
    for th in thresholds:
        # 二值化预测图
        binary_amaps = (amaps > th).astype(np.int32)
        
        # --- 计算 FPR ---
        # FP: 预测为1 但 mask 为 0 的像素
        fp_pixels = np.logical_and(binary_amaps == 1, masks == 0).sum()
        tn_pixels = np.logical_and(binary_amaps == 0, masks == 0).sum()
        
        if (fp_pixels + tn_pixels) > 0:
            fpr = fp_pixels / (fp_pixels + tn_pixels)
        else:
            fpr = 1.0 # 理论上不应该发生
            
        # 如果 FPR 超过 0.3，AUPRO 标准是不计算的，可以提前跳过以加速吗？
        # 通常为了画图还是算完，但在指标计算时截断
        # 这里我们继续计算，但在返回时截断
        ...
        for i in range(len(masks)):
            ...
            # 获取 GT 的连通域
            label_map = measure.label(mask)
            regions = measure.regionprops(label_map)
            
            for region in regions:
                # region.coords: (N_pixels, 2)
    ...
    # 4. 计算 AUC (仅针对 FPR < 0.3 的部分)
    fprs = np.array(fpr_list)
    pros = np.array(pro_list)
    
    # 排序 (roc_curve 出来通常是降序，linspace 是升序，为了 safe 排序一下)
    ...
    # 归一化 FPR (0~0.3 -> 0~1) 以计算面积
    if fprs.size > 1 and fprs.max() > 0:
        fprs_norm = fprs / 0.3
        aupro = auc(fprs_norm, pros)
        return aupro
    else:
        return 0.0
# def compute_pro(masks: ndarray, amaps: ndarray, num_th: int = 200) -> float:
...
#             # 优化：如果是全黑 mask（无异常 GT），直接跳过 regionprops，加速计算
#             if mask.sum() == 0:
#                 continue
...
#         # 计算平均 PRO (处理空列表情况)
#         mean_pro = np.mean(pros) if pros else 0.0
